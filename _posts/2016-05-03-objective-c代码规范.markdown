...
Objective-C是一门面向对象的动态编程语言，主要用于编写iOS和Mac应用程序。关于Objective-C的编码规范，苹果和谷歌以及raywenderlich都已经有很好的总结：
...
不要在工程里使用Tab键，使用空格来进行缩进。在`Xcode > Preferences > Text Editing`将Tab和自动缩进都设置为**4**个空格。
...
命名实例变量，在变量名前加上`_`前缀（*有些有历史的代码会将`_`放在后面*），其它和命名属性一样。不过，除了在初始化方法（`init`、`initWithCoder`等），`dealloc` 方法和自定义的`setter`和`getter`方法中，尽量避免使用变量名前面有下划线（`_variable`）直接访问实例变量。

一般来说，类需要对使用者隐藏数据存储的细节，所以不要将实例方法定义成公共可访问的接口。私有属性应该定义在类的实现文件的类的扩展 (匿名分类) 中。不允许在有名字的 category中定义私有属性，除非你扩展其他类。例如：
...
- (void)viewDidLoad {
    [super viewDidLoad];
	if ([SSKeychain rawLogin].isExist) {
		// 如果用户名密码存在则使用OctoKit APIs去登陆
		// something
		} else {
   		// 去到登陆页面
	}
...
**定义在头文件里的接口方法、属性（特别是数据模型的属性）必须要有注释！**

##其他

###懒加载（Lazy Loading）

如果有必要，最好重写getter方法以实现资源的懒加载。

###init方法

init方法应该遵循Apple生成代码模板的命名规则。返回类型应该使用instancetype而不是id。例如：
...
当类构造方法被使用时，它应该返回类型是instancetype而不是id。这样确保编译器正确地推断结果类型。例如：

```objective-c
@interface Airplane
+ (instancetype)airplaneWithType:(TXAirplaneType)type;
@end
```
...
参考：[Objective-C代码规范](https://github.com/lettleprince/Objective-C-Style-Guide/blob/master/Objective-C代码规范.md)
